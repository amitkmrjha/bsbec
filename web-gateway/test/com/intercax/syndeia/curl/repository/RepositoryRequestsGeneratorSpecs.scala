package com.intercax.syndeia.curl.repository

import org.scalatest.{Matchers, WordSpec}
import play.api.Logger

class RepositoryRequestsGeneratorSpecs extends WordSpec with Matchers {

  val logger = Logger(this.getClass)

  val authToken = "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..bQFo9hoTMZHyLlwkUOUBNW5B7F2RMWryJT7QYX3djdM"
  implicit val authHeader: (String, String) = ("X-Auth-Token", authToken)

  "A RepositoryRequestsGenerator" should {

    "generate curl commands for testing repository endpoints" in {
      val allRequests = RepositoryRequestsGenerator.getAllRequests
      allRequests should not be null
      allRequests should not be empty

      logger.debug(s"${allRequests.mkString(System.lineSeparator())}")
    }
  }
}

