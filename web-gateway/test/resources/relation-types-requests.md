**Create a relation type**

curl --request POST \
  --url http://localhost:9000/types/relation \
  --header 'Content-Type: application/json' \
  --header 'X-Auth-Token: eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..y833ZmRAlU6xMF8BeHLLhCHL5Yr45LfoW4QoEyaRHgA' \
  --data '{
	"name": "rel-type 1",
	"attributeDefinitions": [
		{
			"id": "1",
			"type": "String",
			"metaType": "String",
			"name": "rlt-ad-1",
			"displayName": "rel-type-attrib-def-1",
			"isCollection": false,
			"isModifiable": false,
			"isMandatory": true,
			"defaultValue": {
				"value": "Hello"
			},
			"allowedValues": [
				{
					"value": "Hi"
				},
				{
					"value": "Hey"
				}
			]
		}
	]
}'

**Get a relation type by key**

curl --request GET \
  --url http://localhost:9000/types/relation/REL-TYPE1 \
  --header 'X-Auth-Token: eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..y833ZmRAlU6xMF8BeHLLhCHL5Yr45LfoW4QoEyaRHgA'

**Update relation type by key**

curl --request PUT \
  --url http://localhost:9000/types/relation/REL-TYPE1 \
  --header 'Content-Type: application/json' \
  --header 'X-Auth-Token: eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..W9qaU2IAkUP6vtYbSWMP6Irja47rORr3A1H5YC9UFU8' \
  --data '{
	"name": "rel-type 1 updated",
	"attributeDefinitions": [
		{
			"id": "1",
			"type": "String",
			"metaType": "String",
			"name": "rlt-ad-1",
			"displayName": "rel-type-attrib-def-1",
			"isCollection": true,
			"isModifiable": true,
			"isMandatory": false,
			"defaultValue": {
				"value": "Hello`"
			},
			"allowedValues": [
				{
					"value": "Hi1"
				},
				{
					"value": "Hey1"
				}
			]
		}
	]
}'

**Delete a relation type**

curl --request DELETE \
  --url http://localhost:9000/types/relation/REL-TYPE1 \
  --header 'X-Auth-Token: eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..y833ZmRAlU6xMF8BeHLLhCHL5Yr45LfoW4QoEyaRHgA'

**Get relation type attribute definition by id**

curl --request GET \
  --url http://localhost:9000/types/relation/REL-TYPE1/attribdefs/1 \
  --header ': ' \
  --header 'X-Auth-Token: eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..GDVsb5YJZcFpgXLhSpRtHj6gdgtZC-_9Fep0etL7kbc'
