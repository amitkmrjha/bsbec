**Create a repository type**

curl --request POST \
  --url http://localhost:9000/types/repository \
  --header 'Content-Type: application/json' \
  --header 'X-Auth-Token: eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..y833ZmRAlU6xMF8BeHLLhCHL5Yr45LfoW4QoEyaRHgA' \
  --data '{
	"name": "repo-type 2",
	"attributeDefinitions": [
		{
			"id": "1",
			"type": "String",
			"metaType": "String",
			"name": "rt-ad-2",
			"displayName": "repo-type-attrib-def-2",
			"isCollection": false,
			"isModifiable": false,
			"isMandatory": true,
			"defaultValue": {
				"value": "Hello"
			},
			"allowedValues": [
				{
					"value": "Hi"
				},
				{
					"value": "Hey"
				}
			]
		}
	]
}'

**Get a repository type by key**

curl --request GET \
  --url http://localhost:9000/types/repository/REPO-TYPE1 \
  --header 'X-Auth-Token: eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..y833ZmRAlU6xMF8BeHLLhCHL5Yr45LfoW4QoEyaRHgA'

**Update repository type by key**

curl --request PUT \
  --url http://localhost:9000/types/repository/REPO-TYPE1 \
  --header 'Content-Type: application/json' \
  --header 'X-Auth-Token: eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..W9qaU2IAkUP6vtYbSWMP6Irja47rORr3A1H5YC9UFU8' \
  --data '{
	"name": "repo-type 1 updated",
	"attributeDefinitions": [
		{
			"id": "11",
			"type": "String",
			"metaType": "String",
			"name": "rt-ad-11",
			"displayName": "repo-type-attrib-def-11",
			"isCollection": true,
			"isModifiable": true,
			"isMandatory": false,
			"defaultValue": {
				"value": "Hello1"
			},
			"allowedValues": [
				{
					"value": "Hi1"
				},
				{
					"value": "Hey1"
				}
			]
		}
	]
}'

**Delete a repository type**

curl --request DELETE \
  --url http://localhost:9000/types/repository/REPO-TYPE1 \
  --header 'X-Auth-Token: eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..y833ZmRAlU6xMF8BeHLLhCHL5Yr45LfoW4QoEyaRHgA'

**Get repository type attribute definition by id**

curl --request GET \
  --url http://localhost:9000/types/repository/REPO-TYPE1/attribdefs/1 \
  --header 'X-Auth-Token: eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..GDVsb5YJZcFpgXLhSpRtHj6gdgtZC-_9Fep0etL7kbc'
